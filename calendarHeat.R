...
## This Calendar Heatmap was adapted for Russian language by Kyrylo Zakharov
...
calendarHeat <- function(dates, 
  ...
  caldat$dotw[caldat$dotw == -1] = 6
  caldat$woty <- as.numeric(format(caldat$date.seq, "%U")) + 1
  caldat$woty[caldat$dotw == 6] = caldat$woty[caldat$dotw == 6] - 1
  ...
  print(cal.plot <- levelplot(value~woty*dotw | yr, data=caldat,
                              ...
                              strip=TRUE,
                              main = paste('Календарь "', varname, '"', sep = ""),
                              scales = list(
                                x = list(
                                  at= c(seq(2.9, 52, by=4.42)),
                                  labels = c("Янв", "Фев", "Мар","Апр","Май","Июн","Июл","Авг","Сен","Окт","Ноя","Дек"),
                                  alternating = c(1, rep(0, (nyr-1))),
                                  tck=0,
                                  cex = 0.8),
                                y=list(
                                  at = c(0, 1, 2, 3, 4, 5, 6),
                                  labels = c("Понедельник", "Вторник", "Среда", "Четверг", "Пятница",
                                             "Суббота", "Воскресенье"),
                                  alternating = 1,
                                  cex = 0.8,
                                  tck=0)),
